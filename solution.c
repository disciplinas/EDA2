...
// Região crítica, buffer
File buffer[MAX_FILES];
int used = -1;

pthread_mutex_t mux_buffer;

// Identificar quando producer terminou
...
void produce_aux(char *prefix, char *lookup_dir){
    ...
    while((dp = readdir(dir_ptr)) != NULL){
        ...
        if(stat(filename, &stbuf) == -1){ // deu ruim
        ...
        if((stbuf.st_mode & S_IFMT) == S_IFDIR){ // é um diretório
...
int main(int argc, char** argv){
    ...
    tar_folder();
    nftw(output_dir, rm_r_aux, 64, FTW_DEPTH); // apagar o diretório temporário
